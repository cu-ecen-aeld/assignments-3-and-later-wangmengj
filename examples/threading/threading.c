...
void* threadfunc(void* thread_param)
{
    ...
    // wait wait_to_obtain_ms miliseconds.
    milisleep(pData->wait_to_obtain_ms); 

    // obtain mutex 
    pthread_mutex_lock(pData->mutex);

    // wait wait_to_release_ms miliseconds.
    milisleep(pData->wait_to_release_ms); 
...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    // start thread 
    int iRet = 0 ;
    iRet = pthread_create(thread, NULL, &threadfunc, pData);
    if(0 != iRet)
    {
        free(pData);
        return false;
